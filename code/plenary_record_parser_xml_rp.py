...
BEGIN_STRING = r'^Die\s+Sitzung\s+wird\s+(?:um\s+[0-9]{1,2}[.:][0-9]{1,2}\s+Uhr|nach\s+Behebung\s+eines\s+technischen)|^B\s+e\s+g\s+i\s+n\s+n\s+d\s+e\s+r\s+S\s+i\s+t\s+z\s+u\s+n\s+g(?:\s+)?:(?:\s+[0-9]{1,2}[.:][0-9]{1,2}|\s+[0-9](?:\s+[0-9])?\s+[.:]\s+[0-9]\s+[0-9])'
END_STRING = r'^(?:E\s+n\s+d\s+e\s+d\s+e\s+r\s+S\s+i\s+t\s+z\s+u\s+n\s+g|Ende\s+der\s+Sitzung)(?:\s+)?:(?:\s+[0-9]{1,2}[.:][0-9]{1,2}\s+Uhr|\s+[0-9]\s+[0-9]\s+[.:]\s+[0-9]\s+[0-9])'
CHAIR_STRING = r'^<poi_begin>(Alterspräsident(?:in)?|Präsident(?:in)?|Erste(?:r)?\s+Vizepräsident(?:in)?|Vizepräsident(?:in)?)\s+(.+)(?:\s+\((?:fortfahrend|unterbrechend)\))?:?(?:\s+)?(?:<poi_end>)?'
SPEAKER_STRING = r'^<poi_begin>Abg\.\s+(.+),(?:\s+(?:<poi_end>)?|<poi_end>\s+)(SPD|CDU|FDP|AfD|BÜNDNIS\s+90)'
EXECUTIVE_STRING = r'^<poi_begin>(.+?),\s+(Minister(?:in)?\s+(?:für|der|des)\s+(?:.+)|Ministerpräsident(?:in)?:|Bevollmächtigte(?:r)?.+|Chef(?:in)?\s+der\s+Staatskanzlei)'
...
OMBUDSMAN_MARK = re.compile(r'^<poi_begin>(.+?),\s+Bürgerbeauftragter?')
...
HEADER_MARK = re.compile(r'^Landtag\s+Rheinland-Pfalz\s+-\s+[0-9]{1,2}\.\s+Wahlperiode')
...
for filename in files:
    ...
    wp, session = int(filename[14:16]), int(filename[17:20])
    date = None
