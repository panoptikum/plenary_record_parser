...
# interjection: 87.874
# text x0: 323.150
# interjection: 348.661

# second set of pages:
# text x0: 36.850
# interjection: 62.362
# text: 297.638
# interjection: 323.150
...
def parseXML(xml_in, params, state):
    ...
    NO_INTERJECTION = re.compile(
        r'^.{1,3}' + re.escape(params['closing_mark']))

    BOLD_LS = ['KETJLX', "MOQFYM", "SEUAPU", 'XYYRCW', 'UGSKNF',
               'ELOPTC', 'LMLLKF', 'ADDLZE', 'IKFTEN', 'HGDOQQ']
    ...
    for page in pages:
        ...
        interjection = False
        # import pdb; pdb.set_trace()
        left = [round(float(textbox.attrib["bbox"].split(',')[0:1][0]))
        ...
        if not identation:
            logging.warning(
                'no x0 values within specified ranges' + page.attrib['id'])

        # if page_id=='6':
        #     import pdb; pdb.set_trace()
        x0_occs = []
        for textbox in textboxes:
            x0_occs.append(round(float(textbox.attrib["bbox"].split(',')[0])))
        range_blw_40 = any([i in range(34, 41) for i in x0_occs])
        if range_blw_40:
            interjection_left = params['identation_bound_left_2']
            interjection_right = params['identation_bound_right_2']
        else:
            interjection_left = 87
            interjection_right = 348
        for textbox in textboxes:
            # get the boundaries of the textline
            #import pdb; pdb.set_trace()
            textbox_bounds = [float(s)
                              for s in textbox.attrib["bbox"].split(',')]
            ...
            for line, has_more in lookahead(lines):
                ...
                for char in chars:
                    if poi:
                        if char.attrib:
                            if "Bold" not in char.attrib['font'] and not any(e in char.attrib['font'] for e in BOLD_LS):
                                #import pdb; pdb.set_trace()
                                textbox_text = textbox_text + '<poi_end>'
                                poi = False
                    elif char.attrib:
                        if "Bold" in char.attrib['font'] or any(e in char.attrib['font'] for e in BOLD_LS):
                            #import pdb; pdb.set_trace()
            ...
            textbox = {
                'left': textbox_bounds[0], 'top': textbox_bounds[1], 'text': textbox_text}
            ...
            if textbox['left'] > interjection_left - 3 and textbox['left'] < params['identation_bound_right_1'] - 3:
                textbox['text'] = '<interjection_begin' + \
                    textbox['text'] + '<interjection_end>'
            elif textbox['left'] > interjection_right - 3:
                textbox['text'] = '<interjection_begin' + \
                    textbox['text'] + '<interjection_end>'
...
def iteratesFiles(state):
    ...
    files = [os.path.join(dp, f) for dp, dn, fn in os.walk(
        os.path.expanduser(DATA_PATH)) for f in fn if f.endswith("18_00117.xml")]
    ...
    for filename in sorted(files):
        ...
        with open(filename.replace('.xml', '_xml.txt'), "w", encoding="utf-8") as fp:
